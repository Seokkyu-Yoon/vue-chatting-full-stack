...
RoomManager.prototype.init = function init() {
...
RoomManager.prototype.serialize = function serialize() {
  return Object.keys(this.roomMap).reduce((bucket, roomKey) => {
    const temp = {};
    temp[roomKey] = this.roomMap[roomKey].serialize();
    return {
      ...bucket,
      ...temp,
    };
...
RoomManager.prototype.updateReport = function updateRoom(roomKey) {
  ...
  fs.writeFileSync(room.getRoomPath(), JSON.stringify(room.serialize(), null, 4));
};
...
RoomManager.prototype.join = function join(token, userName, roomKey) {
  if (this.roomMap[roomKey] === undefined) return;
